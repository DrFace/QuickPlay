...
export default function Header({ appName }: { appName: string }) {
        ...
        pages: [
            {
                ...
                startsWith: "/talent",
                isMalty: false,
                isVisible: user ? user['user_type'] == 'client' ? true : false : true,
            },


            {
                name: "E Sports Section",
                href: route("client.talent"),
                startsWith: "/talent",
                isMalty: false,
    ...
    interface GoogleTranslateElement {
        (options: { pageLanguage: string; layout?: any }, id: string): void;
    }

    interface Window {
        googleTranslateElementInit: GoogleTranslateElement;
        google: any;
    }
                                                    ...
                                                    <Dropdown.Trigger>
                                                        <button
                                                            className={` ${url?.startsWith(
                                                                ...
                                                                } px-6 py-3 items-center flex-1 hidden text-sm font-semibold leading-6 text-gray-900 gap-x-1 lg:flex whitespace-nowrap`}
                                                        >
                                                            {page?.name}
                                                    ...
                                                    <Dropdown.Content>
                                                        {1 < 2 ? (
                                                            page?.maltyLinks?.map((item: any) => (
                                                                <Dropdown.Link key={item?.id} href={item?.href} className={` ${item?.textColor ? item?.textColor : 'text-gray-900'} whitespace-nowrap`}>
                                                                    {item?.name}
                                                                </Dropdown.Link>
                                                            ))
                                                        ) : null}
                                                ...
                                                <div key={page?.name} className="flow-root">
                                                    <Link
                                                        ...
                                                        className={` ${url?.startsWith(
                                                            ...
                                                            ? " text-primary-600 border-primary"
                                                            : " text-gray-900 hover:text-primary-600 border-transparent hover:border-primary "
                                                            }  -m-2 block p-2 font-medium whitespace-nowrap`}
                                                    >
                                                        {page?.name}
                                                    </Link>
                                                </div>
                                        ...
                                        <div className="flex justify-start ">
                                            ...
                                            {/* <Link href={route("home")} className="lg:hidden">
                                                ...
                                                <img
                                                    src="/assets/images/ai-geeks.png?a=234"
                                                    alt={appName}
                                                    className="w-auto h-16" // increased from h-10 to h-16
                                                />
                                            </Link> */}
                                                        ...
                                                        {(page?.isMalty && page.isVisible) ? (
                                                            ...
                                                            <Link
                                                                ...
                                                                className={` ${url.startsWith(
                                                                    ...
                                                                    }  hidden text-sm font-semibold leading-6 text-gray-900 lg:flex whitespace-nowrap`}
                                                            >
                                                                {page?.name}
                                                            </Link>
                                                        )}
                                                ...
                                                {page?.isMalty ? (
                                                    <Dropdown>
                                                        <Dropdown.Trigger>
                                                            <button
                                                                className={` ${url.startsWith(
                                                                    ...
                                                                    ? " text-primary-600 border-primary"
                                                                    : " text-gray-900 hover:text-primary-600 border-transparent hover:border-primary "
                                                                    }  -m-2 p-2  border-b-2 items-center flex-1 hidden text-sm font-semibold leading-6 text-gray-900 gap-x-1 lg:flex whitespace-nowrap`}
                                                            >
                                                                {page?.name}
                                                                <ChevronDownIcon className="w-4 h-4" aria-hidden="true" />
                                                            </button>
                                                        ...
                                                        <Dropdown.Content>
                                                            {1 < 2 ? (
                                                                page?.maltyLinks.map((item: any) => (
                                                                    <Dropdown.Link key={item.id} href={item.href} className="whitespace-nowrap">
                                                                        {item.name}
                                                                    </Dropdown.Link>
                                                                ))
                                                            ) : null}
