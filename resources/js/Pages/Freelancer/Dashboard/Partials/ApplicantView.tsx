...
const ApplicantView = ({ job, onClose, user, handleFavoriteJob }: { job: any; onClose: () => void, user: any, handleFavoriteJob: (jobId: string) => void }) => {
  ...
  //  console.log("job", job);
    ...
    return (
        <div className="fixed inset-0 z-20 flex justify-end bg-black bg-opacity-70">
            <div
                ref={modalRef}
                className="w-5/6 h-full overflow-y-auto bg-white shadow-lg md:w-5/6 lg:w-2/3 rounded-tl-2xl"
            >
                <div className="flex items-center justify-between p-6 ">

                    <Link
                        href={route('freelancer.applicant.show', job?.id)}
                    ...
                    <a
                        ...
                        className="flex items-center space-x-2 text-primaryBtnColor hover:text-primaryBtnColorHover"
                    >
                        <span className="text-base font-medium">Open Job in a new window</span>
                ...
                <div className="p-6">
                    <div className="flex flex-col mt-6 sm:flex-row text-start ">
                        <div className="flex flex-col w-full sm:pr-6 sm:w-2/3 ">

                            <div className='flex flex-col gap-4 pb-8 border-b-2'>
                                <div className='flex flex-col gap-4 border-b'>
                                    <h1 className='pb-8 text-xl font-semibold font-Inter'>{job?.title}</h1>
                                </div>
                            ...
                            <div className='flex flex-col gap-4 py-8 border-b-2'>
                                <h1 className='text-xl font-semibold font-Inter'>Category : <span className='font-normal'>{job?.job_category}</span> </h1>
                            </div>
                            <div className='flex flex-col gap-4 py-8 border-b-2'>
                                ...
                                <div className='flex flex-row gap-4'>
                                    ...
                                    <div className='flex flex-row gap-4'>
                                        <div className='flex flex-col gap-2'>
                                            ...
                                            <div>
                                                <p className='text-sm font-normal text-textSecondary font-Inter'>Proposals: {job?.proposal_count >= 2 ? "Less than " + job?.proposal_count : <span className="text-xs ">{job?.proposal_count}</span>}</p>
                                                <p className='text-sm font-normal text-textSecondary font-Inter'>Last updated by client: {job?.updated_at_human_ago}</p>
                        ...
                        <div className="flex flex-col w-full gap-2 px-5 sm:border-l-2 sm:w-1/3 ">
                            <div className="flex flex-col gap-5">
                                <Link className="flex flex-row items-center justify-center w-auto gap-2 px-8 py-3 rounded-full cursor-pointer bg-primaryBtnColor hover:bg-primaryBtnColorHover text-start font-Inter"
                                    href={route('freelancer.proposals.create', job?.id)}>
                                ...
                                <button
                                    ...
                                    className={`flex flex-row items-center justify-center w-auto gap-2 px-8 py-3 rounded-full cursor-pointer text-start font-Inter ${job.is_favorite
                                        ? 'bg-primaryBtnColor text-white'
                                        : 'text-primary border-primary border-2 hover:bg-primaryBtnColorHover hover:text-white' // Default button style
                                        }`}
                                >
                                    <svg
                                        width="17"
                                    ...
                                    >
                                        <path
                                            ...
                                            fill={job.is_favorite ? '#FFFFFF' : 'none'}
                                            stroke="#004AAD"
                                            strokeWidth="1.00189"
                                            strokeLinecap="round"
                                    ...
                                    <span className="text-sm font-medium font-Inter">
                                        {job.is_favorite ? 'Unsave job' : 'Save job'}
                                    </span>
                                </button>
                            ...
                            <div className="flex flex-col gap-2 py-4">
                                ...
                                <p className="text-sm font-normal text-textSecondary text-start font-Inter ">Available Connects : <span className='font-semibold'>{parseFloat(user?.available_connects).toFixed(0)}</span> </p>
                            </div>
                            <div className='flex flex-col'>
                                <AboutClient user={job?.client} />
                    ...
                    <div className="flex flex-row text-start ">
                        <div className="flex flex-col w-full p-6">
                            <div className="flex justify-between gap-4 py-4 ">
                                <div>
                                    <h2 className='text-xl font-medium font-Inter'>Client's recent history</h2>
                                </div>
                            </div>
                            {job?.client?.recent_reviews?.length == 0 ? (
                                <div className="flex flex-col w-full gap-6 text-center">
                                    <h3 className='text-xl font-medium font-Inter'>No recent history</h3>
                                </div>
                            ) : (
                                <>
                                    {job?.client?.recent_reviews?.map((review: any) => (
