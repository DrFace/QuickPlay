...
export default function offerCreate({
   ...
   // console.log(offer);
    ...
    const confirmDelete = () => {
        ...
        router.get(route('freelancer.offer.decline', offer?.id));
    };
                    ...
                    <div className="container flex flex-col gap-4 mx-auto item-center ">
                        ...
                        <div className="flex flex-col gap-6 lg:flex-row">
                            <div className="w-full bg-white lg:w-3/4">
                                ...
                                <div className="flex justify-end w-full px-4 py-10 mt-4 border-t-2">

                                    {offer && offer?.status === 'pending' && (
                                        <div className="flex flex-row items-end w-full gap-4">
                                            <button
                                                type="button"
                                                className="flex items-center justify-center w-full px-6 py-3 text-sm font-medium text-black border rounded-3xl border-primary hover:text-white hover:bg-primary focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-primary"
                                            ...
                                            <Link
                                                href={route('freelancer.offer.accept', offer?.id)}
                                                className="flex items-center justify-center w-full px-6 py-3 text-sm font-medium text-white border border-transparent rounded-3xl bg-primary hover:bg-primaryBtnColorHover focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-primary"
                                            >
                                                Accept Offer
                                            </Link>
                                    ...
                                    {offer && offer?.status === 'accepted' && (
                                        <div className="flex flex-row gap-4">
                                            <Link
                                                href={route('freelancer.contract.show', offer?.id)}
                                    ...
                                    {offer && offer?.status === 'rejected' && (
                                        <div className="flex flex-col items-center w-full gap-4 p-4 text-center bg-red-300 border-2 rounded-2xl">
                                            <span className="text-lg">You have rejected this offer, you can't accept it anymore.</span>
                                        </div>
                                    )}
