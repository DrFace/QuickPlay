...
class ClientReviewController extends Controller
{
    ...
    public function review(string $id)
    {
        ...
        return Inertia::render('Freelancer/Review/Index', [
            'offer' => $offer,
            'user' => Auth::user(),
        ]);
    }
    ...
    public function submitReview(ReviewRequest $request, string $id)
    {
        ...
        $review = $this->reviewInterface->findByColumn(['offer_id' => $offer->id, 'user_id' => $offer->user_id]);
        if ($review && $review->status === 'submitted') {
            ...
            $this->reviewService->ratingUpdate($offer->user_id, $review->avg_score_rate);
        ...
        $this->offerRecentActivityInterface->create([
            'offer_id' => $offer->id,
            'activity' => 'Review submitted by freelancer',
        ]);

        return redirect()->route('freelancer.contract.show', $id)->with('success', 'Review Submitted Successfully');
    }
}
